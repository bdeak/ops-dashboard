...
# get_oncall_data_<lookup_method>()
# get the full name of the oncall from a data storage backend
# and return it

function get_oncall_data_sqlite () {
	global $config, $l;
	...
	$query = "SELECT * from personnel where type='oncall'";
	$result = "N/A";
...
function check_oncall_exists_dashboard_db ($type) {
	global $config;
...
# set the oncall or aod personnel in the dashboard db ($type can be 'aod' or 'oncall')
function set_oncall_sqlite ($type, $username) {
	global $config, $l;

	# get the sqlite connection
	$sqlite = get_sqlite_conn($config["dashboard_db"]);

	if (check_oncall_exists_dashboard_db($type) === true) {
...
function clear_oncall_sqlite ($type, $username) {
	global $config, $l;
