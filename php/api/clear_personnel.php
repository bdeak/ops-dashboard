...
$lookup_func = sprintf("clear_%s_%s", $type, $lookup_method);
...
try {
	call_user_func("$lookup_func", $type);
} catch (Exception $e) {
	handle_error("Can't clear '$type': " . $e->getMessage(), "array");
}
$l->info("Clearing personnel type " . $type . " to " . "'" . $username . "'");
